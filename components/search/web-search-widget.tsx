  ...
  return (
    <div className="h-full flex flex-col bg-transparent min-h-0">
      <div className="relative w-full flex justify-center">
        ...
        <motion.div
          className="absolute top-2 left-1/2 transform -translate-x-1/2 text-purple-500"
          variants={floatVariant}
          animate="float"
        ...
        <motion.div
          className="absolute bottom-2 left-1/2 transform -translate-x-1/2 text-pink-500"
          variants={floatVariant}
          animate="float"
        ...
        {/* Search form with glassmorphism input */}
        <form 
          className="relative w-full mt-12 max-w-2xl"
          ...
          <Input
            ...
            onChange={(e) => setQuery(e.target.value)}
            className={`
              pl-12 pr-12 py-4 text-lg rounded-full
              bg-white/30 backdrop-blur-md
              border border-transparent shadow-[0_0_10px_2px_rgba(236,72,153,0.5),0_0_12px_3px_rgba(59,130,246,0.4)]

              focus:outline-none focus:ring-4 focus:ring-blue-400 focus:ring-opacity-40
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full mt-3">
        ...
        {/* Results container with glassmorphism + gradient + higher z-index */}
        {(webResults.length > 0 || imageResults.length > 0 || loading) && (
          <div
            className="mt-3 bg-white/20 backdrop-blur-md border border-white/30 rounded-lg shadow-lg max-h-[60vh] overflow-y-auto overscroll-contain relative z-30"
            style={{background: 'linear-gradient(135deg, rgba(255,255,255,0.15) 0%, rgba(102,126,234,0.15) 100%)'}}
          >
            {/* Results Info */}
            {(webResults.length > 0 || imageResults.length > 0) && !loading && (
              <div className="flex items-center justify-between text-sm text-gray-600 py-3 px-4 border-b border-gray-200">
