...
export async function GET(request: Request) {
  try {
    ...
    try {
      ...
      const localApiUrl = ` http://locahost:1337/api/breaking-news/live`
      ...
      const pinnedApiUrl = ` http://locahost:1337/api/breaking-news-plural?populate=*&filters[$or][0][isPinned][$eq]=true&filters[$or][1][pinnedAt][$notNull]=true&sort=PublishedTimestamp:desc`
    ...
    let sponsoredData = []
    try {
      const localSponsoredUrl = ` http://locahost:1337/api/sponsored-posts?populate=*&filters[IsActive][$eq]=true`
