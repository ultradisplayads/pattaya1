...
export async function GET(request: Request) {
  ...
  if (contentType === "flight-tracker" || query.toLowerCase().includes("flight") || /^[A-Z]{2}\d+$/.test(query.toUpperCase())) {
    try {
      ...
      const flightResponse = await fetch(` http://locahost:1337/api/flight-trackers`)
  ...
  if (/^[A-Z]{3}$/.test(query.toUpperCase())) {
    try {
      ...
      const flightResponse = await fetch(` http://locahost:1337/api/flight-tracker/airports/${airportCode}`)
